...
gulp.task('clean-invalid', function() {
	return gulp.src([config.directory.invalidDir + '**/*'], {read: false})
...
gulp.task('clean-resize', function() {
	return gulp.src([
		config.directory.dstDir + '/*.' + config.imgConvert.resizeFormat
	], {read: false})
	.pipe(clean({force: true})).on('end', function() {
...
// Delete the resized directory
gulp.task('clean-working-files', function() {
	return gulp.src([config.directory.workDir + '**/*'], {read: false})
	.pipe(clean({force: true}));
});

// Default to clean both validated and invaild folders
gulp.task('clean', ['clean-validate', 'clean-invalid', 'clean-resize', 'clean-working-files']);

// Get all invalid filetypes
gulp.task('checker-invalid', ['clean-invalid'], function() {
	return gulp.src([
...
gulp.task('checker-working-files', ['clean-working-files'], function() {
	return gulp.src([
...
gulp.task('checker-validate', ['clean-validate'], function() {
	return gulp.src([
...
gulp.task('filelist-validated', function(){
	return gulp.src([
		...
		.pipe(gulp.dest(config.directory.validDir + 'data/')).on('end', function() {
			fs.outputJson(config.directory.validDir + 'data/filelist.json', sniff.get("filename"), {spaces: 3}, function (err) {
				if (err !== null) {
					console.error(err);
				}
			});
...
gulp.task('checker', ['checker-working-files', 'checker-invalid', 'checker-validate']);

// Build image resize and convert to desired image format
gulp.task('image-resize', ['clean-resize'], function() {
	return gulp.src([
			config.directory.validDir + '/*.{' + config.validFiletypes.toString() + '}'
		])
		.pipe(plumber())
		...
		.pipe(gulp.dest(config.directory.dstDir)).on('end', function() {
			fs.outputJson(config.directory.dstDir + 'data/filelist.json', sniff.get("resized-filename"), {spaces: 3}, function (err) {
				if (err !== null) {
					console.error(err);
				}
			});
